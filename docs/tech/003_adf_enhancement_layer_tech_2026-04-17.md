...
以下映射基于 PRD v4 Section 4.2 角色增强映射，将"适用阶段"展开为具体 Gate，指定各角色在哪些 Gate 使用哪些增强能力：
...
| Gate 2 Tech Review | brainstorming（方案评审）| plan-eng-review + brainstorming | brainstorming（评审视角）| — | — | — |
| Gate 3 Implementation | — | — | — | review（代码 PR 前）| — | — |
| Gate 4 QA Validation | — | — | qa + qa-only | investigate（Bug 定位）| — | — |
| Gate 5 Release | — | — | — | — | document-release | plan-design-review |

> **说明**：
> - 空白表示该 Gate 该角色不使用增强能力
> - 增强能力使用为**可选**，不作为 Gate 通过的前置条件
> - 增强能力由检测脚本确认已安装后自动可用，无需手动配置
> - 各 Agent 读取 Team config 中的增强层状态，在支持的 Gate 自动调用对应 skill
...
| 2026-04-20 | 架构师 | **HR#1 反馈迭代 v5**：新增检测机制实现逻辑（Section 3）、各 Gate 增强能力使用映射（Section 4.3）| Draft |
