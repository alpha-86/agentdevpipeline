...
- 判定分为三类：
  - `可迁移`：可抽象为通用产研流程机制
  - `部分迁移`：仅迁移机制，必须剥离特定业务语义
  - `禁止迁移`：特定业务专属内容，禁止进入本仓库
...
| `001_Agent团队架构.md` | 部分迁移 | 迁移团队协作框架，剥离领域角色语义 |
| `002_Agent工具设计.md` | 禁止迁移 | 领域数据/研究/验证工具接口专属 |
| `003_产品工程师Agent.md` | 可迁移 | 迁移 PM / Engineer 主链路、PRD 生命周期与 Gate 约束 |
| `004_工作流程规范.md` | 部分迁移 | 迁移流程阶段与治理方法，去特定业务时间切片 |
| `005_人机交互规范.md` | 部分迁移 | 迁移人机交互约束，去 Telegram / 特定业务命令语义 |
| `006_辩论机制与案例.md` | 部分迁移 | 迁移”对抗式评审方法”，去特定业务方案语境 |
| `007_策略研究指南.md` | 禁止迁移 | 领域方案研究专属 |
| `008_因子研究与因子库.md` | 禁止迁移 | 因子体系专属 |
| `009_回测报告设计.md` | 禁止迁移 | 验证范式与业务域指标专属 |
| `010_日报迭代规范.md` | 部分迁移 | 仅迁移“进展/问题/复盘”的日报机制，去业务域字段 |
| `011_文档规范.md` | 可迁移 | 迁移文档契约、命名规范、状态与变更等级 |
| `012_交付物Checklist.md` | 部分迁移 | 迁移交付检查思路，去业务域上线口径 |
| `013_ai_driving_ai.md` | 可迁移 | 迁移多 agent 编排方法 |
| `014_回测报告模板.md` | 禁止迁移 | 验证报告专属 |
...
| `V2.2_双PR机制引入.md` | 部分迁移 | 迁移“双分支/双阶段验收思想”，去特定业务流程绑定 |
...
| `V2.5_DBR例会调整为每日4次.md` | 禁止迁移 | 业务运营节奏专属 |
...
| `ANOMALY_LOOP.md` | 部分迁移 | 迁移异常闭环机制，去业务域告警语义 |
...
| `backtest-agent.md` | 禁止迁移 | 验证业务专属 |
| `bull-agent.md` | 禁止迁移 | 特定业务方案辩论专属 |
| `bear-agent.md` | 禁止迁移 | 特定业务方案辩论专属 |
| `risk-agent.md` | 部分迁移 | 仅迁移工程风险控制方法 |
| `cso.md` | 部分迁移 | 不作为插件主干角色，仅参考通用治理方法 |
| `cro.md` | 禁止迁移 | 业务风险角色专属 |
...
| `strategy-review.md` | 禁止迁移 | 方案业务专属 |
| `strategy-debate.md` | 禁止迁移 | 方案业务专属 |
| `factor-review.md` | 禁止迁移 | 因子业务专属 |
| `backtest-review.md` | 禁止迁移 | 验证业务专属 |
| `risk-report.md` | 部分迁移 | 仅迁移工程风险报告结构 |
| `daily-report.md` | 部分迁移 | 仅迁移通用进展日报结构 |
| `debate.md` | 部分迁移 | 迁移对抗式评审方法，不迁移特定业务内容 |
| `anomaly-iteration.md` | 部分迁移 | 迁移异常迭代机制，不迁移业务域告警字段 |
