...
1. **团队负责人每次收到其他 agent 的反馈后，必须立即根据当前流程执行任务安排、分派下一动作或升级，不得等待，也不得把反馈搁置为无人跟进状态。**
2. 每次会议、评审、专题讨论结束后都必须有正式 纪要。
3. 每个 阻塞项 都必须有 负责人、升级路径和下一次检查时间。
4. 任何角色不得跳过既定 Gate；发现绕过必须立即纠正并留痕。
5. 超期 评审 必须升级，不得以"等一下""稍后看"无限拖延。
6. 每个活跃工作项都必须映射到 issue id、当前 gate 和最近一次决策记录。
7. 会议或评审开始前，必须先读取对应 工作流。
8. 实现阶段 完成后必须立即触发 QA，不得等待手工想起再补。
9. Issue 评论只放结构化结论和链接摘要，不粘贴长篇设计正文。
10. 上下文丢失后必须先恢复 issue、gate、todo 和最新风险，再继续推进。
11. 团队负责人 负责推进和升级，但不得越权替其他角色做正式签字。
12. Agent 之间的正式交接必须以文件交付物和结构化评论为准，不得依赖聊天上下文接力。
13. **若其他 agent 的反馈改变了当前阶段判断、暴露 blocker、或补齐了准入条件，团队负责人必须在同一轮内更新任务安排，明确“谁下一步做什么”。**
